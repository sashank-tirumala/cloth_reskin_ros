...
class GraspServer(object):
    ...
    def manual_grasp(self, msg):
        ...
        rospy.loginfo(
            "Received grasp request with %f height diff and %f slide_diff"
            % (height_diff, slide_diff)
        )
    ...
    def create_exp_dir(self):
        ...
        self.trial_type_dir = "%s/%s/%s_exp%d" % (
            self.save_dir,
            self.exp_name,
            self.trial_type,
            run_idx,
    ...
    def baseline_grasp(self, msg, random=False):
        ...
        for i in range(self.num_attempts):
            ...
            if random:
                ...
                if goal_layers == 1:
                    height_absolute = 0.1575 + np.random.uniform(low=-0.005, high=0.005)
                elif goal_layers == 2:
                    height_absolute = 0.14 + np.random.uniform(low=-0.005, high=0.005)
    ...
    def closedloop_grasp(self, msg):
        ...
        for i in range(self.num_attempts):
            ...
            if constants.RANDOM_HEIGHT_FLAG:
                increment = constants.INCREMENT + np.random.normal(
    ...
    def run_experiment(self, msg):
        self.num_attempts = rospy.get_param(
