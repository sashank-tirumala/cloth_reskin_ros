    ...
    for i in range(len(plan.joint_trajectory.points)):
        ...
        for j in range(len(plan.joint_trajectory.points[i].positions)):
            ...
            accelerations.append(
                plan.joint_trajectory.points[i].accelerations[j] * scale * scale
            )
...
class MoveActionServer:
    def __init__(self):
        ...
        self.group = moveit_commander.MoveGroupCommander(
            "panda_arm", robot_description="robot_description", ns=""
        )
        ...
        self.movefingers_client = actionlib.SimpleActionClient(
            "move_fingers", MoveFingersAction
        )
        self.movefingers_client.wait_for_server()

        self.reset_server = actionlib.SimpleActionServer(
            "reset", ResetAction, execute_cb=self.reset, auto_start=False
        )
        self.reset_server.start()

        self.moveandgrasp_server = actionlib.SimpleActionServer(
            "move_and_grasp",
            MoveAndGraspAction,
            execute_cb=self.move_action,
            auto_start=False,
        )
        self.moveandgrasp_server.start()
        self.min_depth = 0.108
        self.init_pose = None
    ...
    def update_height(self, init_pose, height_diff):
        ...
        if (
            pose.position.z - height_diff < self.min_depth
        ):  # account for min valid depth
            clipped_height_diff = pose.position.z - self.min_depth
        else:
            clipped_height_diff = height_diff
        rospy.loginfo(
            "clipped height diff: %f, %f" % (clipped_height_diff, self.min_depth)
        )
    ...
    def update_trajectory(self, init_pose, post_action=False):
        ...
        paramlist = rosparam.load_file(
            "/home/tweng/catkin_ws/src/cloth_reskin_ros/singlearm_pkg/config/config.yaml"
        )
    ...
    def move(
        ...
        if height_diff == -100.0 and height_absolute != -100.0:  # no support for NaN
        ...
        (plan, fraction) = self.group.compute_cartesian_path(
            waypoints, 0.01, 5.00  # waypoints to follow, eef_step, jump_threshold
        )
        if fraction != 1.0:
    ...
    def move_action(self, goal):
        ...
        rospy.loginfo(
            "Received %s request with grasp params height diff %s"
            % (command, height_diff)
        )
        if height_diff != -100.0 and abs(height_diff) > 0.3:
        ...
        if command == "move":
            if not self.move(
                height_diff, height_absolute=height_absolute, slide_diff=slide_diff
            ):
                self.moveandgrasp_server.set_aborted(move_result)
                return
