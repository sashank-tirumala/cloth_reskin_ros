...
class GraspServer(object):
    def __init__(self):
        ...
        self.im_clf_sub = rospy.Subscriber("/img_classifier_output", Int64, self.img_clf_callback)
    ...
    def init_rand_exp(self):
        ...
        if self.rand_exp_folder == "":
            ...
            with open('%s/%s/exp_cfg.yaml' % (self.save_dir, self.rand_exp_folder), 'w') as yaml_file:
            ...
            with open('%s/%s/exp_cfg.yaml' % (self.save_dir, self.rand_exp_folder), 'r') as yaml_file:
    ...
    def run_experiment(self, msg):
        ...
        for name in self.method_names:
            ...
            if num_trials < self.num_trials_per_method:
                ...
                rospy.logerr('More than %d trials for method %s' % (self.num_trials_per_method, name))
        ...
        for method_name in np.random.permutation(methods_to_sample):
            if 'image' in method_name and not rospy.get_param('/robot_app/imgclf_ready'):
                continue
            else:
                break
    ...
    def baseline_grasp(self, goal_layers, random=False):
        ...
        success = False
        num_attempts = rospy.get_param('/robot_app/num_exp_attempts') # number of attempts per trial
        ...
        for i in range(num_attempts):
            ...
            if random:
                ...
                # height_absolute = np.random.uniform(low=0.135, high=0.165) # horizontal
                ...
                if goal_layers == 1:
                    ...
                    # vertical
                    # height_absolute = 0.238

                    # height_absolute = openloop[0]
                    if ol_noise != 0:
                        rospy.logwarn("Using noisy open loop")
                    height_absolute = openloop[0] + np.random.uniform(low=-ol_noise, high=+ol_noise)
                elif goal_layers == 2:
                    # horizontal
                    # height_absolute = .14 # + np.random.uniform(low=-0.005, high=0.005)
                    # vertical

                    # height_absolute = openloop[1]
                    if ol_noise != 0:
                        rospy.logwarn("Using noisy open loop")
                    height_absolute = openloop[1] + np.random.uniform(low=-ol_noise, high=+ol_noise)
                elif goal_layers == 3: # mainly for data collection
                    if ol_noise != 0:
                        rospy.logwarn("Using noisy open loop")
                    height_absolute = openloop[2] + np.random.uniform(low=-ol_noise, high=+ol_noise)

                rospy.loginfo("open loop height: %f" % height_absolute)
            # TODO remove slide diff, no longer used
            slide_diff = -0.04
            ...
            pred = val[index]
            rospy.logwarn("pred: %d goal: %d num preds: %d" % (pred, goal_layers, len(clf_preds)))
        ...
        if clf_pred_success:
            # Robustify grasp
            # self.move_and_grasp(command="robustify", height_diff=0, slide_diff=0)
    ...
    def closedloop_grasp(self, goal_layers):
        ...
        # height_absolute = .167 # + np.random.uniform(low=-0.005, high=0.005)
        closedloop = rospy.get_param('robot_app/closedloop')
        height_absolute = closedloop[0] if goal_layers == 1 else closedloop[1] # + np.random.uniform(low=-0.005, high=0.005) # TODO set height to be the same with random range
        ...
        for i in range(num_attempts):
            ...
            if(constants.RANDOM_HEIGHT_FLAG):
                increment += np.random.normal(loc=0.0, scale=0.0001) #Adding randomness to height movement (Does it improve or not?). Reduced to 0.003
            ...
            if pred == goal_layers:
                ...
                if pred > goal_layers: # move up
                    # self.move_and_grasp(command='movedown', height_diff=0.02, slide_diff=None)
                    height_diff -= increment
