...
class Classifier:
    def __init__(self):
        ...
        self.reskin_sub = rospy.Subscriber("/reskin", Float64MultiArray, self.reskin_callback)        
        # self.reskin_data = np.zeros([self.num_pts, 15])
    ...
    def spin(self):
        while not rospy.is_shutdown():
            if(FLAG_ALWAYS_CLASSIFY):
                reskin_dat = np.array(self.reskin_data)#Automatically makes a copy, fastest way for small data
                ...
                self.pub.publish(Int64(int(np.around(np.mean(y_pred))))) # TODO maybe change for single data case
            ...
            # rospy.loginfo("Predicted num layers: %f" % y_pred)

    def reskin_callback(self, data):
        cdat = data.data
        cnumpy = np.array([cdat[1], cdat[2], cdat[3], cdat[5], cdat[6], cdat[7], 
        cdat[9], cdat[10], cdat[11], cdat[13], cdat[14], 
        cdat[15], cdat[17], cdat[18], cdat[19]])
        self.reskin_data = cnumpy[np.newaxis, :]

        # if(self.data_counter < self.num_pts):
        #     self.reskin_data[self.data_counter,:] = cnumpy
        #     self.data_counter=self.data_counter+1
        # if(self.data_counter >= self.num_pts):
        #     self.reskin_data = np.roll(self.reskin_data, shift=-1, axis=0)
        #     self.reskin_data[-1,:] = cnumpy
