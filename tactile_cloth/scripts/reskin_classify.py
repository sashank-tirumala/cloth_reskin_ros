...
def classify_reskin_img(cloth0_dir_names, cloth1_dir_names, cloth2_dir_names, n_folds,
        debug_print=True):
    ...
    X_data, Y, x_data, y, _ = U.get_dataset_from_dir_names(cloth0_dir_names, cloth1_dir_names,
        cloth2_dir_names, args.n_epis, args.n_train, get_images=True)
    ...
    if 'franka_norub_folded_random_18feb' in cloth0_dir_names[0]:
        X_train = X_train[:, 20:460, 20:620, :]  # crop more uniformly
        X_valid = X_valid[:, 20:460, 20:620, :]  # crop more uniformly
    else:
        X_train = X_train[:, 120:, 40:480, :]  # crop part to right (+tiny bit to left), crop top
        X_valid = X_valid[:, 120:, 40:480, :]  # crop part to right (+tiny bit to left), crop top
    Y_train = Y
    Y_valid = y
    ...
    # Use the ImageFolder data format, also helps us inspect the labels.
    ...
    if debug_print:
        print(model_ft)
    print(f'Parameter count of loaded ResNet: {U.count_parameters(model_ft)}')
