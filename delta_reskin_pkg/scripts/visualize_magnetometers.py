...
def plot_all_data(bagfile, fname, has_prediction=False):
    ...
    pred = []
    topics = (
        ["/reskin", "/webcam_image", "/classifier_output", "/classifier_commands"]
        if has_prediction
        else ["/reskin", "/webcam_image"]
    )
...
def create_video_array(bagfile, CUTOFF, has_prediction=False, plot_progress=False):
    ...
    if plot_progress:
        ...
        bar = progressbar.ProgressBar(
            ...
            widgets=[
                progressbar.Bar("=", "[", "]"),
                " making magnetometers video ",
                progressbar.Percentage(),
            ],
        )
        bar.start()
    topics = (
        ["/reskin", "/webcam_image", "/classifier_output"]
        if has_prediction
        else ["/reskin", "/webcam_image"]
    )
    ...
    for topic, msg, t in bagfile.read_messages(topics=topics):
        ...
        if topic == "/reskin":
            ...
            axes[5].set_ylim(-1.5, 3.5)
            axes[5].scatter(
                [reskin_dmsecs[0], reskin_dmsecs[-1]], [-1, -1], color="white"
            )
            if len(clf_preds) != 0:
                axes[5].plot(clf_dmsecs, clf_preds)
...
if __name__ == "__main__":
    ...
    video = cv2.VideoWriter(
        "/home/sashank/catkin_ws/src/tactilecloth/videos/"
        + args.name
        + "_magnetometers.mp4",
        fourcc,
        10,
