...
class DeltaZ:
    def __init__(self):
        ...
        self.delta_actionserver = actionlib.SimpleActionServer(
            "move_fingers",
            MoveFingersAction,
            execute_cb=self.run_command,
            auto_start=False,
        )
    ...
    def execute_pinch(self):
        ...
        if rospy.get_param("/app/random_grasp_pos") == 1:
            stop = False
            while not stop:
                yrand, xrand = np.random.uniform(
                    low=-0.75, high=0.75, size=2
                )  # changed from 1
    ...
    def execute_dbg(self):
        ...
        sleep(0.1)
        final_pose = np.array(
            [push_force, y_offset, 3 + z_forward, -(push_force), 0, 3 + z_forward]
        )
        self.clf_pub.publish("start")  # start classifier
        sleep(0.5)
        self.clf_pub.publish("end")  # start classifier
    ...
    def execute_moveforward(self):
        ...
        push_force_reduce = 1
        final_pose = np.array(
            [push_force - push_force_reduce, y_offset, 3, -(push_force), 0, 3]
        )
        move_to_xyz_pos(final_pose, self.delta_message, self.arduino)
        sleep(0.1)
        final_pose = np.array(
            [push_force - push_force_reduce, y_offset, 3, -push_force, 0, 3 + z_forward]
        )
        move_to_xyz_pos(final_pose, self.delta_message, self.arduino)
        sleep(0.1)
    ...
    def joy_callback(self, data):
        ...
        self.trials_to_collect = (
            self.num_auto_trials if self.collection_mode == "auto" else 1
        )
