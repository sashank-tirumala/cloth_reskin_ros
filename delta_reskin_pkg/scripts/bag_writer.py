...
class RosbagWriter:
    def __init__(self):
        ...
        self.success_sub = rospy.Subscriber(
            "/success_trial", String, self.success_callback
        )
    ...
    def create_trial_bag(self, data):
        ...
        # Init bag
        self.bag_name = "%s/%s/data.bag" % (self.trial_type_dir, self.trial_name)
    ...
    def save_cfg(self, data):
        ...
        params = [
            x for x in names if "/app" in x or "/robot_app" in x or "/exp_cfg" in x
        ]
        for key in params:
            data_dict[key] = rospy.get_param(key)

        with open(
            "%s/%s/config.yaml" % (self.trial_type_dir, self.trial_name), "w"
        ) as yaml_file:
            yaml.dump(data_dict, yaml_file, default_flow_style=False)

    def success_callback(self, msg):
        with open(
            "%s/%s/success_%s.txt" % (self.trial_type_dir, self.trial_name, msg.data),
            "w",
        ) as txt_file:
    ...
    def record_callback(self, msg):
        if msg.data.startswith("start"):
            ...
            if self.proc is not None:
                rospy.logerr(
                    "Error: ovewriting proc var, killing old proc, should probably exit"
                )
