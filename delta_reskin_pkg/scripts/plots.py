...
def juxtaposition_plot(data1, data2, figname):
    ...
    fig.text(
        0.04, 0.5, "Magnetometer Data", va="center", rotation="vertical", fontsize=16.0
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--path", help="path to trial", type=str, default=None)
    args = parser.parse_args()

    trial_data = get_trial_data(args.path)
    plot_magnetometer(data=trial_data, fname=args.path + "/magnetometers.png")
    plot_magnetometer_by_row(
        data=trial_data, fname=args.path + "/magnetometers_by_component.png"
    )
