...
def get_data_XY_new(dir_name, get_images=False):
    ...
    training. Will need to confirm with Sashank about this. That's embedded in
    the `np.linspace()` which assumes equally spaced intervals.
...
def plot_magnetometer(data):
    ...
    fig.text(
        0.04, 0.5, "Magnetometer Data", va="center", rotation="vertical", fontsize=14.0
    )
...
# deprecated?
def create_cloth_datas(
    cloth0_dir_names, cloth1_dir_names, cloth2_dir_names, max_x_len, new_collect=True
):
...
def get_dataset_dir_names(cloth0, cloth1, cloth2, num_train):
    ...
    train_dir_names = cloth0[0:num_train] + cloth1[0:num_train] + cloth2[0:num_train]
    test_dir_names = cloth0[num_train:] + cloth1[num_train:] + cloth2[num_train:]
...
def get_dataset_from_dir_names(
    cloth0_dir_names,
    cloth1_dir_names,
    cloth2_dir_names,
    n_epis,
    ...
    print(f"Train/Test epis (c0): {tr_c0}, {te_c0}")
    print(f"Train/Test epis (c1): {tr_c1}, {te_c1}")
    print(f"Train/Test epis (c2): {tr_c2}, {te_c2}")
    cloth0_shuffled = []
    cloth1_shuffled = []
    cloth2_shuffled = []
    ...
    X, Y, x, y, info = get_raw_data(
        train_dir_names, test_dir_names, new_collect, get_images=get_images
    )
...
def pad_time_series(data, new_len):
    ...
    for j in range(data.shape[1]):
        new_data[:, j] = np.pad(
            data[:, j], (left_pad_values, right_pad_values), mode="edge"
        )
    return new_data
...
def debug_transf_images(data_imgs, is_valid=False):
    ...
    for o_img in orig_imgs:
        ...
        cv2.imwrite(f"transf_img_debug_{is_valid}_{c}.png", o_img)
...
def _get_processed_reskin(reskin_c1, reskin_c2, n_epis, n_train, ignore_zero=False):
    ...
    print(f"Train, valid epis (c1): {t_idxs_c1}, {v_idxs_c1}")
    print(f"Train, valid epis (c2): {t_idxs_c2}, {v_idxs_c2}")
    X_train, y_train = [], []
    X_valid, y_valid = [], []
...
def classify_reskin(
    reskin_c1, reskin_c2, n_folds=50, debug_print=True, ignore_zero=True
):
    ...
    for i in range(n_folds):
        ...
        data_np = _get_processed_reskin(
            reskin_c1, reskin_c2, n_epis, n_train, ignore_zero=ignore_zero
        )
        ...
        if debug_print:
            ...
            print(f"Total train + valid: {len(y_train)+len(y_valid)}")
        ...
        print(f"Balanced accuracy score: {score:0.3f}")
        print(best_cf)
