...
def classify_reskin_img(cloth0_dir_names, cloth1_dir_names, cloth2_dir_names, args):
    ...
    X_data, Y_train, x_data, Y_valid, _ = U.get_dataset_from_dir_names(
        cloth0_dir_names,
        cloth1_dir_names,
        cloth2_dir_names,
        args.n_epis,
    ...
    X_train, _ = (
        X_data[:, :480, :, :],
        X_data[:, 480:, :, :],
    )  # (images, matplotlib) upper
    X_valid, _ = (
        x_data[:, :480, :, :],
        x_data[:, 480:, :, :],
    )  # (images, matplotlib) lower
    ...
    best_bacc = balanced_accuracy_score(
        valid_gt, valid_preds_best
    )  # shapes are: (n_valid,)
    ...
    print(
        f"See the PyTorch model/optim state dicts here with other stats:\n\t{savepath}"
    )
...
def classify_reskin(
    ...
    for i in range(n_folds):
        ...
        if test_drift == 1:
            ...
            _, _, X_test_d, y_test_d, info = U.get_raw_data(
                train_dir_names=[],
                test_dir_names=cloth0_dir_names,
                new_collect=True,
                get_images=False,
            )
            test_lens = [tup[0] for tup in info]
            ...
            first2_acc = first2_correct / (
                first2_correct + best_cf[0, 1] + best_cf[1, 0]
            )
        ...
        if gradually_increase:
            ...
            for seed in range(10):
                ...
                for perc in percentages:
                    ...
                    X_subset_train, X_subset_test = U.normalize_data(
                        X_subset_train, X_test_orig
                    )
                    ...
                    best_cf = confusion_matrix(y_test_orig, y_pred)
                    print(
                        f"  {perc:0.4f}, X: {X_subset_train.shape}, score: {score:0.4f}"
                    )
                    # print(f'{best_cf}\n')
...
if __name__ == "__main__":
    ...
    if args.method in ["knn", "rf"]:
        ...
        classify_reskin_img(
            cloth0_dir_names, cloth1_dir_names, cloth2_dir_names, args=args
        )
    else:
